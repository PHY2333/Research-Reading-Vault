...
- merge_to_main: 首次交付时未合并；2026-09-13 用户已明确授权后续合并
...
首次交付时，任务分支内的写作、审查、应用与索引工作已完成并推送，按默认规则停在合并前。

2026-09-13 用户随后明确回复“全部同步”，授权合并本任务并提交其余 29 个现有本地文件。同步分支为 `codex/sync-vault-20260913`，目标为本地和 GitHub main。S009 v1 的来源登记、全文译稿及 23 张截图进行文件／链接完整性检查后按现有内容提交；保留既有“已通读”“已核对”及无待核对项的记录，未重做全文翻译验收，未改变主辅关系。Obsidian 三份 JSON 均有效。原 Notes 任务目录与独立工作树继续保留，不另建前置笔记或同步任务目录。
