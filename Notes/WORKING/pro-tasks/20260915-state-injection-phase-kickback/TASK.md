...
用户最新明确要求“codex把相位回踢部分的注释加入”。依此覆盖默认 Pro 概念写作路由，由 Codex 直接添加一条短小局部注释，不重新调用 Pro。沿用已完成的异或版正文，只在 §2.1 的 CNOT 双重描述后补充默认展开的 callout；不改写其它正文。

用户后续指出核心应是“代入并按 P_± 合并”。据此将注释改为算符推导：从 d⊗a 顺序的计算基控制表达式出发，定义数据线投影 P_±，代入 I=P_++P_-、X=P_+−P_-，按投影分组后识别辅助线的 I 与 Z。相位回踢缩为末尾简短解释，callout 默认展开。
...
实际落盘的投影分解与合并推导已经独立局部核对；Obsidian 数学检查和 git diff --check 通过。开始本轮时工作区已存在 §1.2 “根据测量结果 m”的措辞修改，完整保留为未提交状态，本轮仅暂存 callout 修订及任务记录。
