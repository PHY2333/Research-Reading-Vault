...
所以后文可以把相位函数按模 $1$ 比较。若 $f-g$ 模 $1$ 是一个常数，则两个门只相差全局相位。
...
第二层就是 Clifford 门：共轭任何 Pauli 后仍得到 Pauli，允许相差全局相位。第三层要求共轭任何 Pauli 后得到 Clifford 门。这里的层数衡量这种**递归共轭条件**。
...
按空乘积约定 $\boldsymbol a=0$ , $q_{\varnothing}=1$给出

$$
U_{m,0}=e^{2\pi i/2^m}I.
$$

它只是含全局相位的恒等门，属于所有正整数编号的层。若也为它记录最低正整数编号，该编号是 $1$；不能把非空支持的公式套过来，称它的最低层数为 $m-1$。
...
因为括号中的 $QPQ^\dagger$ 仍是 Pauli。反向推导只需对 $VQ$ 右乘 $Q^\dagger$。

注意，这里只证明了**右乘 Pauli**不改变层级归属，没有把任意两个同层门的乘积当作同层门,事实上两个同层门的乘积只有在 Pauli群和 Clifford群中等于同层门。后面的上界证明也不会假定 $k\ge3$ 的整个 $\mathcal C_k$ 是群。
...
相位函数换一个模 $1$ 的代表，不影响这个判据：相差整数值的函数的差分仍是相差整数值的函数。给原门增加全局相位对应于给 $f$ 加一个常数，而常数的差分直接为零。
...
这证明了一个不依赖层级分类的计算事实：**对任意方向 $\boldsymbol u$，一个计数为 $w=m+r-1$ 的单项式相位经过差分后，模 $1$ 可以写成常数加若干整数倍的非空支持单项式相位，而每个剩余项的计数至多是 $w-1$。** 当 $w=1$ 时，只剩常数相位。
...
这里 $N$ 有限，也允许为零。整数系数代表重复相乘和逆门，因为
...
这里允许方向重复。“模 $1$ 为常数”指它给每个计算基态相同的相位。
...
两个情形都得到了相对相位，而不只是一个非零的常数相位。证明只在支持内选择方向，所以加入不参与相位的比特也不会改变下界。
