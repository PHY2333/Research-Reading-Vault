...
上一节中，$\alpha^2$ 与 $\alpha+1$ 是两种表达式，但约化规则要求它们表示同一个元素。要把这种计算变成严格的构造，我们先在多项式的集合中规定哪些写法应当合并，再把合并后得到的每一组作为一个新元素。
...
我们希望构造一个元素 $\alpha$，使 $f(\alpha)=0$。这条要求会迫使某些多项式表达式取相同的值：若 $p-q=fh$，其中 $p,q,h\in\mathbb F_2[x]$，那么在满足所需关系的算术中就应有
...
因此，相差 $f$ 的倍数的多项式必须表示同一个元素。我们就以此作为构造中的合并规则：恰好把差为 $f$ 的倍数的多项式归入同一组，记为
...
这里右边只涉及已有的多项式运算，可以直接判断，不需要先找到 $\alpha$。这个关系具有自反性、对称性与传递性：对应的差分别是 $0$、原来差的相反数，以及两个差的和，它们仍然是 $f$ 的倍数。因此它是一个**等价关系**，能把多项式分成互不相交的组。

包含 $p$ 的那一组称为它的**等价类**，记为

$$
[p]=\{p+fh:h\in\mathbb F_2[x]\}.
$$

我们把整个类 $[p]$ 作为新集合中的一个元素，而多项式 $p$ 是它的一个**代表元**。换成 $p+fh$，仍然是在用另一种写法指向同一个类。

例如，取上一节对应的 $f=x^2+x+1$。在 $\mathbb F_2[x]$ 中，

$$
x^2-(x+1)=f,
$$

所以 $[x^2]=[x+1]$。这正是把“平方项与一次式表示同一个元素”落实为新集合中的相等关系。同样，$[f]=[0]$，因为 $f-0$ 是 $f$ 的倍数。
...
这里的商记号表示按上述关系合并多项式。现在新集合已经构造出来；接下来要在这些类上定义加法和乘法，再取 $\alpha=[x]$，验证它确实满足 $f(\alpha)=0$。
...
这个条件可以转化为求逆算法。目标是找到多项式 $u,v$，使

$$
ug+vf=1.
$$

这样的等式称为**贝祖等式**。它把 $1$ 写成 $g$ 与 $f$ 的多项式倍数之和；转到商中以后，$vf$ 会变成零，剩下的 $u$ 就能给出 $g$ 的逆元。下面从带余除法实际找出这两个系数。

先处理 $g=1$：直接取 $u=1,v=0$ 即可。以下设 $\deg g\ge1$，记

$$
r_0=f,\qquad r_1=g.
$$

用 $r_1$ 去除 $r_0$，得到商 $q_1$ 和余式 $r_2$；再用 $r_2$ 去除 $r_1$，如此继续。每一步的等式为

$$
r_{i-1}=q_i r_i+r_{i+1},
\qquad
r_{i+1}=0\ \text{或}\ \deg r_{i+1}<\deg r_i.
$$

其中 $q_i$ 也是 $\mathbb F_2[x]$ 中的多项式。非零余式的次数是严格下降的非负整数，所以过程必定停止。设最后一个非零余式为 $r_m$，则整条除法链写成
...
这里最后一步的余式已经是零；需要保留的是此前的非零余式 $r_m$。

若第一次余式就等于 $1$，则 $m=2$，上面的首尾等式会重合，整条链只有 $f=q_1g+1$、$g=q_2\cdot1$ 两步。此时从第一步移项，就已经得到只含 $f,g$ 的等式 $1=f-q_1g$。

为什么 $r_m$ 就是最大公因式？由每一步的等式，任何同时整除 $r_{i-1},r_i$ 的多项式，也整除 $r_{i+1}=r_{i-1}-q_i r_i$；反过来，同时整除 $r_i,r_{i+1}$ 的多项式，也整除 $r_{i-1}$。所以相邻两项的公因子集合在每步除法后保持不变。最后一对是 $(r_m,0)$，其最大公因式为 $r_m$，因而

$$
\gcd(f,g)=\gcd(r_1,r_2)=\cdots=\gcd(r_m,0)=r_m.
$$

我们已知 $\gcd(f,g)=1$，因此 $r_m=1$。在 $\mathbb F_2$ 中，非零常数只有 $1$，这里无需再调整常数因子。

现在从**产生余式 $1$ 的那一步**开始倒代。将其移项，得到

$$
1=r_m=r_{m-2}-q_{m-1}r_{m-1}.
$$

右侧若还有中间余式，就用产生它的除法等式替换。例如，每个 $r_j$ 都可以写成
...
从右侧下标最大的余式开始替换，再合并同类项，每次都会将它换成下标更小的余式。重复这个过程，最终只剩最初的 $r_0=f$ 和 $r_1=g$，于是得到

$$
1=ug+vf.
$$

替换过程中只做多项式加减和乘法，因此最后 $g,f$ 前的系数 $u,v$ 仍是多项式；它们就是所求的贝祖系数。保留除法链并据此倒代求出系数的方法，称为**扩展欧几里得算法**。上面用减号表示移项，在 $\mathbb F_2[x]$ 中实际计算时，加法与减法相同。
...
上面的除法只产生一次非零余式，就已经得到 $1$。再求 $\alpha^2$ 的逆元，可以看见中间余式如何在倒代中被消去。

取 $g=x^2$，仍用同一个 $f=x^3+x+1$。先用 $g$ 去除 $f$，再用所得余式去除 $g$：
...
这里的商依次是 $x,x+1,x+1$，非零余式依次是 $x+1,1$。第二行可以直接展开核对：$(x+1)^2+1=x^2+1+1=x^2=g$。

现在从第二行得到的余式 $1$ 开始。先移项，再用第一行的 $x+1=f+xg$ 替换余式，最后按 $g,f$ 收集系数：
...
&=g+(x+1)(f+xg)\\
&=g+(x+1)f+x(x+1)g\\
&=\bigl(1+x(x+1)\bigr)g+(x+1)f\\
&=(x^2+x+1)g+(x+1)f.
\end{aligned}
$$

第二行只替换前一行中作为余式的那个 $x+1$；另一个 $x+1$ 是除法得到的商，继续作为多项式系数保留。这样，每一步替换都保留了原来的等式。

至此，右侧已经没有中间余式，可以直接读出

$$
u=x^2+x+1,\qquad v=x+1.
$$

转到商中，$[f]=0$，而 $[g]=\alpha^2$，所以

$$
\boxed{(\alpha^2)^{-1}=\alpha^2+\alpha+1}.
$$
...
&=(\alpha^2+\alpha)+(\alpha+1)+\alpha^2\\
&=1.
\end{aligned}
$$

这里用了 $\alpha^4=\alpha\alpha^3=\alpha(\alpha+1)=\alpha^2+\alpha$。整个求逆过程由此连接起来：带余除法找到最后一个非零余式 $1$，倒代把它写成 $ug+vf$，取类后得到 $[u][g]=1$。
